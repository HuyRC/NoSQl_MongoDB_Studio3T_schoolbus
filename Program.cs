...
app.MapPut("/api/schools/{id}", async (string id, School s) =>
{
    ...
    var r = await ctx.Schools.ReplaceOneAsync(x => x.Id == s.Id, s);
    return r.MatchedCount == 0 ? Results.NotFound() : Results.NoContent();
});

app.MapDelete("/api/schools/{id}", async (string id) =>
{
    var r = await ctx.Schools.DeleteOneAsync(x => x.Id == ObjectId.Parse(id));
...
{
    s.Id = ObjectId.Parse(id);
    var r = await ctx.Students.ReplaceOneAsync(x => x.Id == s.Id, s);
    return r.MatchedCount == 0 ? Results.NotFound() : Results.NoContent();
});

app.MapDelete("/api/students/{id}", async (string id) =>
{
    var r = await ctx.Students.DeleteOneAsync(x => x.Id == ObjectId.Parse(id));
    return r.DeletedCount == 0 ? Results.NotFound() : Results.NoContent();
});
...
{
    s.Id = ObjectId.Parse(id);
    var r = await ctx.Stops.ReplaceOneAsync(x => x.Id == s.Id, s);
    return r.MatchedCount == 0 ? Results.NotFound() : Results.NoContent();
});

app.MapDelete("/api/stops/{id}", async (string id) =>
{
    var r = await ctx.Stops.DeleteOneAsync(x => x.Id == ObjectId.Parse(id));
    return r.DeletedCount == 0 ? Results.NotFound() : Results.NoContent();
});
...
{
    r.Id = ObjectId.Parse(id);
    var res = await ctx.Routes.ReplaceOneAsync(x => x.Id == r.Id, r);
    return res.MatchedCount == 0 ? Results.NotFound() : Results.NoContent();
});

app.MapDelete("/api/routes/{id}", async (string id) =>
{
    var r = await ctx.Routes.DeleteOneAsync(x => x.Id == ObjectId.Parse(id));
    return r.DeletedCount == 0 ? Results.NotFound() : Results.NoContent();
});
...
{
    d.Id = ObjectId.Parse(id);
    var r = await ctx.Drivers.ReplaceOneAsync(x => x.Id == d.Id, d);
    return r.MatchedCount == 0 ? Results.NotFound() : Results.NoContent();
});

app.MapDelete("/api/drivers/{id}", async (string id) =>
{
    var r = await ctx.Drivers.DeleteOneAsync(x => x.Id == ObjectId.Parse(id));
    return r.DeletedCount == 0 ? Results.NotFound() : Results.NoContent();
});
...
{
    b.Id = ObjectId.Parse(id);
    var r = await ctx.Buses.ReplaceOneAsync(x => x.Id == b.Id, b);
    return r.MatchedCount == 0 ? Results.NotFound() : Results.NoContent();
});

app.MapDelete("/api/buses/{id}", async (string id) =>
{
    var r = await ctx.Buses.DeleteOneAsync(x => x.Id == ObjectId.Parse(id));
    return r.DeletedCount == 0 ? Results.NotFound() : Results.NoContent();
});
...
app.MapPut("/api/trips/{id}", async (string id, Trip t) =>
{
    ...
    var r = await ctx.Trips.ReplaceOneAsync(x => x.Id == t.Id, t);
    return r.MatchedCount == 0 ? Results.NotFound() : Results.NoContent();
});

app.MapDelete("/api/trips/{id}", async (string id) =>
{
    var r = await ctx.Trips.DeleteOneAsync(x => x.Id == ObjectId.Parse(id));
    return r.DeletedCount == 0 ? Results.NotFound() : Results.NoContent();
});
...
{
    var a = await ctx.Assignments.Find(x => x.Id == ObjectId.Parse(id)).FirstOrDefaultAsync();
...
{
    a.Id = ObjectId.Parse(id);
    var r = await ctx.Assignments.ReplaceOneAsync(x => x.Id == a.Id, a);
    return r.MatchedCount == 0 ? Results.NotFound() : Results.NoContent();
});

app.MapDelete("/api/assignments/{id}", async (string id) =>
{
    var r = await ctx.Assignments.DeleteOneAsync(x => x.Id == ObjectId.Parse(id));
    return r.DeletedCount == 0 ? Results.NotFound() : Results.NoContent();
});
